...
// Expert-facing reference: normative conformance criteria, reference config,
// how compliance is verified, and normative references. Merged onto STANDARDS.
const TECH = {
  "SEC-001": {
  ...
  "step_up": { "actions": ["break_glass", "role_assume"], "max_age": "0s" }
    ...
    verify: [
      "Auth logs are asserted in CI: every prod session carries amr ∈ {hwk, fido}.",
...
window.SP_DATA.STANDARDS.forEach((s) => { s.tech = TECH[s.id] || null; });
window.SP_DATA.TECH = TECH;
