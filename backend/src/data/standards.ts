import type { Category, Standard, Enabler, Compliance, StatusMap, ComplianceLevel, StatusKey, CoverageLevel } from './types';
...
export const STANDARDS: Standard[] = [
  {
    ...
    summary: 'All customer and internal data must be encrypted while moving across networks and while stored on disk, using current, approved algorithms.',
    impactHeadline: 'One unencrypted database is the difference between a non-event and a front-page breach.',
    impact: [
    ...
    exceptions: 'Legacy systems unable to support TLS 1.2 may request a time-boxed exception (max 90 days) with a compensating control and a documented remediation date, approved by the Security guild.',
    history: [
  ...
  },
  {
    ...
    summary: 'Any access to production systems, source control, or customer data requires phishing-resistant multi-factor authentication.',
    impactHeadline: 'Over 80% of breaches start with a stolen password. MFA closes that door.',
    impact: [
    ...
    exceptions: 'Service accounts use scoped, rotated credentials in the secrets manager in lieu of MFA, reviewed quarterly.',
    history: [
  ...
  },
  {
    ...
    summary: 'Every dataset must carry a sensitivity label (Public, Internal, Confidential, Restricted) that drives how it can be accessed, shared and retained.',
    impactHeadline: 'You can\'t protect, or delete, what you can\'t find. Classification makes data governable.',
    impact: [
    ...
    exceptions: 'Short-lived analytics scratch space is exempt for up to 14 days, after which data must be labelled or purged.',
    history: [
  ...
  },
  {
    ...
    summary: 'Each dataset should declare how long it is kept and what happens at end of life, so data isn\'t retained indefinitely by default.',
    impactHeadline: 'Data you no longer need is pure liability, storage cost today, breach exposure tomorrow.',
    impact: [
      { stat: '$310K/yr', label: 'storage reclaimed in the first year' },
      { stat: 'Lower', label: 'legal exposure in discovery and disputes' },
    ],
    why: 'Holding data "just in case" quietly accrues cost and risk. Clear retention shrinks our storage bill, limits what can be exposed in a breach, and keeps us from producing years of irrelevant data in litigation. It also signals to customers that we are deliberate stewards of their information.',
    practice: [
      'Declare retention in the data catalog at dataset creation.',
      'Automate deletion or archival at end of life where possible.',
    ],
    related: ['DATA-002'],
    exceptions: 'Data under legal hold is retained until the hold is lifted, regardless of policy.',
    history: [
  ...
  },
  {
    ...
    summary: 'Production services must publish Service Level Objectives and track an error budget that governs the balance between shipping features and protecting reliability.',
    impactHeadline: 'SLOs turn "is it down?" arguments into a shared number everyone can act on.',
    impact: [
    ...
    exceptions: 'Internal tools with no external SLA may track a single availability SLO only.',
    history: [
  ...
  },
  {
    ...
    summary: 'Every significant incident should get a written, blameless review with action items and an owner, completed within five business days.',
    impactHeadline: 'An incident you don\'t learn from, you pay for twice.',
    impact: [
      { stat: '3x', label: 'more repeat incidents on systems without reviews' },
      { stat: 'Trust', label: 'blameless culture keeps reporting honest and fast' },
    ],
    why: 'The most expensive incident is the one that recurs. Blameless reviews surface the systemic causes (not the scapegoats) so we fix the conditions that produced the failure. Keeping them blameless is what keeps engineers reporting problems early instead of hiding them until they explode.',
    practice: [
  ...
  },
  {
    ...
    summary: 'Public and partner-facing APIs must be explicitly versioned, with backward-compatible changes only inside a version and a published deprecation path.',
    impactHeadline: 'A silent breaking change is an outage you ship to your customers on purpose.',
    impact: [
    ...
    exceptions: 'Internal-only APIs may evolve faster by agreement between producer and all consumers.',
    history: [
  ...
  },
  {
    ...
    summary: 'Services should ship a machine-readable OpenAPI description that stays in sync with the running code.',
    impactHeadline: 'Documentation that drifts from reality is worse than none, schemas keep them honest.',
    impact: [
      { stat: 'Days to hours', label: 'partner integration time with generated SDKs' },
      { stat: 'Self-serve', label: 'fewer engineer hours answering "how do I call this?"' },
    ],
    why: 'A current OpenAPI schema is a force multiplier: it generates client SDKs, powers interactive docs, and lets partners self-serve instead of filing tickets. Generating it from code means it never silently drifts out of date, the most common and most damaging failure mode of hand-written docs.',
    practice: [
  ...
  },
  {
    ...
    summary: 'All customer-facing interfaces must meet WCAG 2.2 Level AA, so the product is usable with assistive technology, keyboards, and a range of vision and motor abilities.',
    impactHeadline: 'Accessibility isn\'t a feature for a few, it\'s the law, a market, and just better engineering.',
    impact: [
    ...
    why: 'Inaccessible products exclude a sixth of the population and expose us to real legal action under the ADA and the European Accessibility Act. But the upside is bigger than the downside: accessible engineering, semantic markup, keyboard support, sufficient contrast, produces interfaces that are faster, more robust, and easier for everyone, including users on bad networks and small screens.',
    practice: [
    ...
    related: ['FE-005'],
    exceptions: 'Internal admin tools target AA but may phase in over two quarters with a tracked plan.',
    history: [
  ...
  },
  {
    ...
    summary: 'Key pages should set and enforce a performance budget (size and Core Web Vitals) so speed doesn\'t quietly regress over time.',
    impactHeadline: 'Every 100ms of latency is measurable revenue walking out the door.',
    impact: [
      { stat: '+1%', label: 'conversion for every 100ms faster on checkout' },
      { stat: 'SEO', label: 'Core Web Vitals directly affect search ranking' },
    ],
    why: 'Speed is conversion. Slow pages lose customers before they ever see the product, and search engines now rank on Core Web Vitals, so performance is also acquisition. A budget enforced in CI stops the slow, invisible bloat that otherwise creeps in one library at a time.',
    practice: [
  ...
  },
  {
    ...
    version: '2.2',
    updated: '2026-04-14',
    owner: { name: 'Tomas Vidal', role: 'Head of Infrastructure', team: 'Infrastructure', email: 'infra-guild@acme.dev' },
    appliesTo: ['Infrastructure', 'Platform', 'Payments', 'ML Systems'],
    summary: 'Cloud resources must be created and changed through version-controlled infrastructure-as-code, never by manual console changes.',
    impactHeadline: 'Click-ops can\'t be reviewed, reproduced, or rolled back. Code can.',
    impact: [
    ...
    exceptions: 'Break-glass console changes are permitted during incidents and must be codified within 48 hours.',
    history: [
  ...
  },
  {
    ...
    summary: 'Every cloud resource should carry tags identifying its owning team, cost center and environment.',
    impactHeadline: 'Untagged spend is spend nobody is accountable for, and it always grows.',
    impact: [
    ...
    why: 'When cloud cost can\'t be attributed, nobody owns it and it balloons. Consistent tagging turns the monthly bill into per-team accountability, makes idle resources visible, and lets finance forecast accurately instead of guessing. It\'s the smallest possible change with an outsized impact on the bottom line.',
    practice: [
  ...
  },
  {
    ...
    summary: 'Customer-facing decisions or content produced by AI models must have a defined human-review or appeal path before they take material effect.',
    impactHeadline: 'A model\'s mistake is the company\'s mistake, keep a human in the loop where it matters.',
    impact: [
    ...
    why: 'Models are powerful but confidently wrong in ways that can harm customers and the brand. For high-stakes decisions (eligibility, moderation, financial actions) a human checkpoint or appeal route protects customers, limits our liability, and keeps us ahead of regulation like the EU AI Act rather than scrambling to comply later.',
    practice: [
    ...
    exceptions: 'Low-risk uses (e.g. drafting suggestions a human accepts) need disclosure but not pre-review.',
    history: [
  ...
  },
  {
    ...
    summary: 'Models affecting people should be tested for disparate impact across protected groups, both before launch and continuously in production.',
    impactHeadline: 'A biased model scales unfairness at machine speed, measure it before it ships.',
    impact: [
    ...
    why: 'Bias in a model isn\'t a one-off error, it\'s systematic harm applied to everyone it touches. Measuring disparate impact protects customers from unfair outcomes and gives us defensible evidence if a decision is ever challenged. Continuous monitoring catches drift, because a model fair at launch can become unfair as the world changes.',
    practice: [
  ...
  },
  {
    ...
    summary: 'API keys, tokens and credentials must live in the central secrets manager, never in source code, config files, or environment files committed to a repo.',
    impactHeadline: 'A secret in a Git history is a secret you\'ve already lost.',
    impact: [
      { stat: 'Hours to never', label: 'secrets that used to leak via repos, now don\'t' },
      { stat: 'Instant', label: 'rotation when a credential is suspected compromised' },
    ],
    why: 'Credentials committed to source control leak constantly (through forks, public mirrors, and laptops) and live forever in history. Centralising secrets means they can be rotated instantly, access can be revoked in one place, and we get an audit trail of who used what. It removes an entire category of self-inflicted breaches.',
    practice: [
  ...
  },
  {
    ...
    summary: 'Teams should perform and document a full restore-from-backup test every quarter to prove backups actually work.',
    impactHeadline: 'A backup you\'ve never restored is a hope, not a plan.',
    impact: [
      { stat: 'Proven', label: 'recovery, not assumed recovery' },
    ],
    why: 'Superseded by REL-001\'s broader recoverability requirements, which fold backup-restore testing into error-budget and disaster-recovery objectives. Retained here for historical reference.',
    practice: [
...
export const ENABLERS: Enabler[] = [
  ...
  },
  {
    ...
    owner: { name: 'Tomas Vidal', role: 'Head of Infrastructure', email: 'infra-guild@acme.dev' },
...
// Derive coverage rank and attach enabler to each standard
const COV_RANK: Record<CoverageLevel, number> = { full: 0, most: 1, partial: 2 };
STANDARDS.forEach((s) => {
  let best: Enabler | null = null;
  let bestCov: CoverageLevel | null = null;
  ENABLERS.forEach((e) => {
    const m = e.meets.find((x) => x.id === s.id);
    if (m && (best === null || COV_RANK[m.covers] < COV_RANK[bestCov!])) {
      best = e;
      bestCov = m.covers;
    }
  });
  s.enabler = best ? { ...(best as Enabler), covers: (bestCov as unknown) as CoverageLevel, desc: (best as Enabler).what } : null;
});

// Derive category counts
CATEGORIES.forEach((c) => {
  c.count = STANDARDS.filter((s) => s.category === c.id && s.status !== 'deprecated').length;
});
